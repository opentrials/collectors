...
class Mapper(base.Mapper):
    ...
    def map_response(self, res):
        ...
        get = lambda path: res.css(path).extract_first()  # noqa
        value = []
        for ctl in ['01', '02']:
            value.append({
        ...
        path = '//*[re:test(@id, "DataList8_ctl\d+_Condition_FreeTextLabel")]/text()'  # noqa
        value = res.xpath(path).extract()
        data[key] = value

        key = 'interventions'
        path = '//*[re:test(@id, "DataList10_ctl\d+_Intervention_FreeTextLabel")]/text()'  # noqa
        value = res.xpath(path).extract()
        data[key] = value

        key = 'primary_outcomes'
        path = '//*[re:test(@id, "DataList12_ctl\d+_Outcome_NameLabel")]/text()'  # noqa
        value = res.xpath(path).extract()
        data[key] = value

        key = 'secondary_outcomes'
        path = '//*[re:test(@id, "DataList14_ctl\d+_Outcome_NameLabel")]/text()'  # noqa
        value = res.xpath(path).extract()
        data[key] = value
        ...
        path = '//*[re:test(@id, "DataList20_ctl\d+_Secondary_SponsorLabel")]/text()'  # noqa
        value = res.xpath(path).extract()
        data[key] = value
